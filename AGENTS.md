...
- [2026-05-20] Run Animation v3.0 (sprite_2.webp, 146 frames): Refine `Run_Animation.html` với 3-phase animation đồng bộ beer button. Phase RUN (frames 1–27, index 0–26): loop liên tục khi `loading` (FPS=24). Phase STOP (frames 28–123, index 27–122): chạy 1 lần không loop từ frame hiện tại → frame 122 khi `ready`, tự động chuyển sang CHEER. Phase CHEER (frames 124–146, index 123–145): loop liên tục + CSS bounce sau khi complete (FPS=18). Xử lý edge case: nếu frame đang ở ngoài vùng STOP thì nhảy thẳng vào CHEER. Sprite URL: `sprite_2.webp`, COLS=8, FRAME_W/H=128px.
