...
- [2026-05-20] Multi-Instance Beer Button Sync: Sửa lỗi nút #btn-video-loading bị treo trạng thái loading bằng cách sử dụng `document.currentScript.parentNode` để truy vấn phần tử cục bộ (local elements) thay vì `document.getElementById` toàn cục. Phát CustomEvent đồng bộ tất cả các widget trùng lặp trên giao diện Elementor (Desktop/Mobile).
